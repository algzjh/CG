...
public:
    ...
    // (i.e., mapped textures)
    Vec3d ke( const isect& i ) const { return _ke.value(i); } //emissive 发射
    Vec3d ka( const isect& i ) const { return _ka.value(i); } //ambient 环境
    Vec3d ks( const isect& i ) const { return _ks.value(i); } //specular 镜面
    Vec3d kd( const isect& i ) const { return _kd.value(i); } //diffuse 漫射
    Vec3d kr( const isect& i ) const { return _kr.value(i); } //reflective 反射
    Vec3d kt( const isect& i ) const { return _kt.value(i); } //transmissive 透射

	//需要传入交点，因为需要交点来计算参数坐标

    double shininess( const isect& i ) const //亮度指数 n_s
	{
    ...
    double index( const isect& i ) const { return _index.intensityValue(i); } //折射系数
